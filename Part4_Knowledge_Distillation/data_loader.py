...
def get_data_paths():
    path_train_nir = './newdatasets/NIR'
    path_train_rgb = './newdatasets/RGB-Registered'
    path_val = './newdatasets/Validation'

    # nir_files_png1 = glob.glob(path_train_nir + '/*_nir.png')
    nir_files_png1 = glob.glob(path_train_nir + '/NIR*.png')
    nir_files = sorted(nir_files_png1 )

    # nir_files_png1_val = glob.glob(os.path.join(path_val, '*_nir.png'))
    nir_files_png1_val = glob.glob(os.path.join(path_val, 'NIR_*.png'))
    nir_files_val = sorted(nir_files_png1_val )

    # rgb_files_png = glob.glob(path_train_rgb + '/*reg.png')
    rgb_files_png = glob.glob(path_train_rgb + '/RGBR*.jpg')
    rgb_files = sorted( rgb_files_png)

    # rgb_files_png_val = sorted(glob.glob(os.path.join(path_val, '*_rgb_reg.png')))
    rgb_files_png_val = sorted(glob.glob(os.path.join(path_val, 'RGBR*.jpg')))
...
def get_test_paths():
    path_val = './newdatasets/Testing'
    # nir_files_png2 = sorted(glob.glob(os.path.join(path_val, '*nir_reg.png')))
    nir_files_png2 = sorted(glob.glob(os.path.join(path_val, 'NIR*.png')))
    nir_files_val = sorted(nir_files_png2)

    # rgb_files_png = sorted(glob.glob(os.path.join(path_val, '*rgb_reg.png')))
